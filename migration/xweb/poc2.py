...
def check(v, i, o, s, x, m, f, fp, classic, odin):
    ...
    if(exclude(domain, x)):
        print ("Excluding domain: " + domain + " from dns updates."); return;
    ...
    with(open(fp)) as thisFile:
		...
		if(re.search(pattern, myfile)):
			if(classic): myfile=re.sub(pattern, m[key], myfile);
			elif(odin): myfile=re.sub(pattern, m[key]+"\n", myfile);
			write_output=True
    ...
    if write_output:
        if(v): print("file: " + f + " domain: " + f + " updated")
...
def main():
    ...
    classic=False; odin=False;
    exclude=False; exclude_file=None;
    zonedir_in=None; zonedir_out=None;
    verbose=True
    ...
    for o, a in opts:
	    ...
	    if(a == "classic"): classic=True;
	    elif(a == "odin"): odin=True;
	    else: errmsg += "Environmentment flag not valid\n";
        else:
