        ...
        for key in m:
            ...
            if key in myfile:
                ...
                if(re.search(pattern, myfile)):
                    ...
                    writeZoneFile=True
			#try to handle odin reverse zone files
			if(odin):
				key_in_arpa = key[::-1]
				val_in_arpa = m[::-1]
				if key_in_arpa in myfile:
					pattern="^"+key_in_arpa+".IN-ADDR.ARPA."
					myfile=re.sub(pattern, val_in_arpa+".IN-ADDR.ARPA.", myfile)
					writeArpaFile=True
					print ("doing an arpa update")
